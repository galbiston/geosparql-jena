...
import queryrewrite.expr.WithinExprFunc;
 ...
 * @author haozhechen
 */
public class WithinQRPropertyFunc extends PropertyFunctionBase {
    ...
    public QueryIterator exec(Binding binding, PropFuncArg argSubject, Node predicate, PropFuncArg argObject, ExecutionContext execCxt) {
        ...
        Expr expr = new WithinExprFunc(new ExprVar(GMLVar_SUB.getName()), new ExprVar(GMLVar_OBJ.getName()));

        if (nodeVar_SUB.isLiteral()) {
            if (nodeVar_OBJ.isLiteral()) {
                //Subject is GML literal, Object is GML Literal

                expr = new WithinExprFunc(exprVar_SUB, exprVar_OBJ);
            } else if (nodeVar_OBJ.isURI()) {
                //Subject is GML literal, Object is Feature
                bp.add(FeaHasGeom_OBJ);
                bp.add(GeomHasGML_OBJ);

                expr = new WithinExprFunc(exprVar_SUB, new ExprVar(GMLVar_OBJ.getName()));
            } else {
            ...
            if (nodeVar_OBJ.isLiteral()) {
                ...
                expr = new WithinExprFunc(new ExprVar(GMLVar_SUB.getName()), exprVar_OBJ);
            } else if (nodeVar_OBJ.isURI()) {
