...
import queryrewrite.expr.OverlapsExprFunc;
 ...
 * @author haozhechen
 */
public class OverlapsQRPropertyFunc extends PropertyFunctionBase {
    ...
    public QueryIterator exec(Binding binding, PropFuncArg argSubject, Node predicate, PropFuncArg argObject, ExecutionContext execCxt) {
        ...
        Expr expr = new OverlapsExprFunc(new ExprVar(GMLVar_SUB.getName()), new ExprVar(GMLVar_OBJ.getName()));

        if (nodeVar_SUB.isLiteral()) {
            if (nodeVar_OBJ.isLiteral()) {
                //Subject is GML literal, Object is GML Literal

                expr = new OverlapsExprFunc(exprVar_SUB, exprVar_OBJ);
            } else if (nodeVar_OBJ.isURI()) {
                //Subject is GML literal, Object is Feature
                bp.add(FeaHasGeom_OBJ);
                bp.add(GeomHasGML_OBJ);

                expr = new OverlapsExprFunc(exprVar_SUB, new ExprVar(GMLVar_OBJ.getName()));
            } else {
            ...
            if (nodeVar_OBJ.isLiteral()) {
                ...
                expr = new OverlapsExprFunc(new ExprVar(GMLVar_SUB.getName()), exprVar_OBJ);
            } else if (nodeVar_OBJ.isURI()) {
