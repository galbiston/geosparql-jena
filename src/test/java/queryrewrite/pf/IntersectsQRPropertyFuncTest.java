 ...
 * @author haozhechen
 */
public class IntersectsQRPropertyFuncTest {

    private static Model MODEL;
    private static InfModel INF_MODEL;
    private static final Logger LOGGER = LoggerFactory.getLogger(IntersectsQRPropertyFuncTest.class);

    public IntersectsQRPropertyFuncTest() {
    }
    ...
    /**
     * Test of exec method, of class SFIntersectsQRPropertyFunc.
     */
    @Test
    public void testExec() {
        ...
        String queryString = "SELECT ?place WHERE{ "
                + "ntu:F ntu:hasExactGeometry ?dGeom . ?dGeom gml:asGML ?dGML . "
                + "?place ntu:hasExactGeometry ?Geom . ?Geom gml:asGML ?GML . "
                + "?dGML geo:sfIntersects ?GML ."
                + "FILTER ( ?Geom != ?dGeom )"
                + " }";
